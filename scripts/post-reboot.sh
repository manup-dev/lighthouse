...
#   2. Ensures the 14B model is loaded on GPU
...
curl -s -X POST http://127.0.0.1:11434/api/chat \
  ...
  -d '{"model":"qwen2.5:14b-instruct-q4_K_M","messages":[{"role":"user","content":"hi"}],"stream":false,"options":{"num_ctx":16384}}' \
...
# Use 14B on GPU with bigger ctx — we have the budget now.
